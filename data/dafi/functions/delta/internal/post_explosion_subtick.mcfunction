#> dafi:delta/internal/post_explosion_subtick
#   Runs comands as the player immediately after the creeper explosion has been processed
#
# Callers:
#   > dafi:delta/post_explosion_trigger (advancement)

advancement revoke @s only dafi:delta/post_explosion_trigger
...
execute as @e[tag=delta.shifted] at @s run function dafi:delta/internal/unshift_entity
execute as @a[tag=delta.ignored_player] run function dafi:delta/internal/end_ignore_player