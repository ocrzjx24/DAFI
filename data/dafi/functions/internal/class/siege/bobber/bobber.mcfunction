...
execute at @s as @e[type=marker,tag=dafi.siege.bobbercheck,tag=dafi.siege.markerunprocessed,sort=nearest] store result score @s dafi.sbsid run scoreboard players get @e[type=fishing_bobber,tag=dafi.siege.bobbing,limit=1] dafi.sbsid 
...
# execute as @e[type=marker,tag=dafi.siege.bobbercheck,scores={bobberCheckLifetime=1},sort=nearest] if score @s dafi.sbsid = @e[type=marker,tag=dafi.siege.bobbing,limit=1] dafi.sbsid run say help
