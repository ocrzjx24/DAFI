...
execute if score @s dafi.saint.dropFeather matches 120 run item replace entity @s hotbar.1 with feather{display:{Name:'[{"text":"Wings","italic":false,"bold":true,"color":"white"}]'},Enchantments:[{id:"unbreaking",lvl:1}],Unbreakable:1,HideFlags:1} 1
...
# increment/reset
execute if score @s dafi.saint.dropFeather matches 120.. run scoreboard players reset @s dafi.saint.dropFeather
...
# SOUND
execute if score @s dafi.saint.dropFeather matches 119 at @s run playsound minecraft:block.beacon.activate master @a[distance=..10] ~ ~1000 ~ 1000 1 1

# KILL
kill @e[type=item,nbt={Item:{id:"minecraft:feather"}}]

# DEATHCHECK
execute if score @s dafi.saint.dropFeather matches 1.. if score @s dafi.deathcheck matches 1.. run execute if entity @s run scoreboard players set @s dafi.saint.dropFeather 199
