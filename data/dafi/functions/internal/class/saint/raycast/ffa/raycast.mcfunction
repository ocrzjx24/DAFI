...
execute as @a[distance=..3] run effect give @s slow_falling 1 0 true
execute as @a[distance=..3] run effect give @s levitation 1 0 true
scoreboard players set @a[distance=..3] delta.addition.magnitude 20000
execute as @a[distance=..3] at @s rotated as @a[tag=dafi.saint.this,sort=nearest,limit=1] run function dafi:internal/mechanics/addition/add_motion
