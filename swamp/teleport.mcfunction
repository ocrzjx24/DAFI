...
execute as @e[type=armor_stand,tag=swampTP] if score @s sbsid = @a[scores={swampTPTrigger=1},limit=1] sbsid run tp @a[scores={swampTPTrigger=1}] @s
execute as @e[type=armor_stand,tag=swampTP] if score @s sbsid = @a[scores={swampTPTrigger=1},limit=1] sbsid run kill @s
